...
muBall = sqrt(2)*sqrt((log(1/epsilon.PE) + numel(params.expectationsJoint)*log(m+1))/m);
...
mathSolverInput.vectorOneNormConstraints = VectorOneNormConstraint(params.observablesJoint(:),params.expectationsJoint(:),muBall+params.t);
...
debugInfo.storeInfo("t",params.t)
...
function [keyRate, debugInfo] = finiteKeyRateRenyi(relEnt, deltaLeak, ~, params, ~, debugInfo)
...
ECLeakage = n/N*deltaLeak + ceil(log2(2/epsilon.EC))/N; 
